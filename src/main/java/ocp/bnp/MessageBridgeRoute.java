...
public class MessageBridgeRoute extends RouteBuilder {
    ...
    public void configure() throws Exception {
        rest().post("/message").id("rest").to("direct:publish");

        from("direct:publish").id("publish").setBody().simple("Este es un mensaje de prueba para AMQ")
                .log("Enviando mensaje" + body()).to("amq:queue:{{amq.queue}}").log("Mensaje Enviado ");

        from("amq:queue:{{amq.queue}}").id("amq").log("ActiveMQ received: ${body}");
